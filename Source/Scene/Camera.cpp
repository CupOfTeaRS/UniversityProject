...
void CCamera::ViewOnlyTechnique()
{
	if (KeyHit(Key_1))
	{
		cameraViewMethod = PlainTexture;
	}
	if (KeyHit(Key_2))
	{
		cameraViewMethod = PixelLit;
	}
	if (KeyHit(Key_3))
	{
		cameraViewMethod = PixelLitTex;
	}
	if (KeyHit(Key_4))
	{
		cameraViewMethod = AlphaBlend;
	}
	if (KeyHit(Key_5))
	{
		cameraViewMethod = GodRays;
	}
	if (KeyHit(Key_6))
	{
		cameraViewMethod = DepthOnly;
	}
}
...
CVector2 CCamera::PixelFromWorldPtGetPt(CVector3 worldPt, TUInt32 ViewportWidth, TUInt32 ViewportHeight)
{
	TInt32 X;
	TInt32 Y;
	CVector3 viewportPoint = m_MatViewProj.TransformPoint(worldPt);
	if (viewportPoint.z < 0)
	{
		return CVector2(0, 0);
	}
	viewportPoint.x /= viewportPoint.z;
	viewportPoint.y /= viewportPoint.z;

	X = static_cast<TInt32>((viewportPoint.x + 1.0f) * ViewportWidth * 0.5f);
	Y = static_cast<TInt32>((1.0f - viewportPoint.y) * ViewportHeight * 0.5f);
