...
void CParseLevel::EntitiesStartElt( const string& eltName, SAttribute* attrs )
{
	...
	else if (eltName == "Randomise")
	{
		...
		m_Pos.y += Random( 0.0f, randomY );
		m_Pos.z += Random( 0.0f, randomZ );
	}
}
...
if (m_EntityType == "Player")
{
	...
	m_EntityManager->CreatePlayer(m_EntityType, m_EntityName, m_Pos, m_Rot, m_Scale);
}
if (m_EntityType == "Player2")
{
	m_EntityManager->CreatePlayer(m_EntityType, m_EntityName, m_Pos, m_Rot, m_Scale);
}
}
